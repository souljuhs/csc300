...
public class Polling {
	...
	public static ArrayList<String> readCauses(String fileName) throws IOException {
		...
		// debugger: prints the absolute file path
	...
	public static int[][] performSurvey(ArrayList<String> causesList, int numPolled) {
		...
		for (int i = 0; i < numPolled; i++) {
			...
			for (int j = 0; j < causesList.size(); j++) {
				polling[j][i] = randy.nextInt(1, 11); // ensures valid range from 1-10
	...
	public static void reportResults(ArrayList<String> causesList, int[][] pollResults, double[] averageForCause,
			int[] causesSum) {
		...
		outputWriter.printf("The cause %s with an average rating of %.2f is the highest rated cause%n",
				causesList.get(maxIndex), averageForCause[maxIndex]);
		outputWriter.printf("The cause %s with an average rating of %.2f is the lowest rated cause%n",
				causesList.get(minIndex), averageForCause[minIndex]);
	}
